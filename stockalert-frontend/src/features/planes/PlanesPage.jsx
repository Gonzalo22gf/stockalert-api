  ...
  return (
    <div className="space-y-8 animate-rise">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        {PLANES.map((plan) => (
          <div key={plan.id} className={"relative rounded-2xl border p-6 flex flex-col gap-4 " + (plan.destacado ? "border-brand-400/40 bg-brand-500/5" : "border-slate-800 bg-white/[0.02]")}>
            ...
            <ul className="space-y-2 flex-1">
              {plan.features.map((f) => <li key={f} className="flex items-center gap-2 text-sm text-slate-300"><span className="text-brand-400">✓</span>{t(f)}</li>)}
            </ul>
            <button onClick={() => irACheckout(plan.id)} disabled={cargando === plan.id} className={"w-full rounded-xl py-3 text-sm font-semibold transition-colors " + (plan.destacado ? "bg-brand-500 text-white hover:bg-brand-400" : "border border-slate-700 text-slate-300 hover:bg-slate-800")}>
