...
export default function Topbar({ titulo, subtitulo, onToggleSidebar }) {
  ...
  function ir(ruta) { setMenuAbierto(false); navigate(ruta); }

  const chipClase = "hidden lg:flex items-center gap-1.5 rounded-[9px] border border-border bg-panel px-3 py-[7px] text-[12.5px] font-medium text-slate-400 transition-all hover:bg-panel-hover hover:border-slate-700";
  ...
  return (
    <header className="relative flex items-center justify-between gap-2 border-b border-border-soft bg-base/80 px-4 py-3 backdrop-blur-xl md:px-7 md:py-4">
      <div className="flex min-w-0 items-center gap-3">
        <button onClick={onToggleSidebar} title="Mostrar/ocultar menu" className="shrink-0 rounded-lg border border-border bg-panel p-2 text-slate-400 transition-all hover:bg-panel-hover hover:text-white">
          <Menu size={18} />
        </button>
            ...
            <>
              ...
              <div className="absolute right-0 z-20 mt-2 w-52 animate-pop overflow-hidden rounded-xl border border-[#2a2e3a] shadow-2xl shadow-black/60" style={{ backgroundColor: "#13151c" }}>
                ...
                {accesos.filter((a) => !a.soloAdmin || esAdmin).map((a) => {
                  const Icono = a.icono;
                  return (
                    <button key={a.ruta} onClick={() => ir(a.ruta)} className="flex w-full items-center gap-2.5 px-4 py-2.5 text-left text-[13px] font-medium transition-colors hover:bg-[#1a1d26]" style={{ color: "#cbd1e0" }}>
