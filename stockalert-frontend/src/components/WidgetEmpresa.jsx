    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="rounded-2xl border border-border-soft bg-panel p-4 space-y-3">
        ...
        <div className="flex items-center gap-3">
          <p className="text-2xl font-bold tracking-widest text-brand-400">
            {empresa?.codigoAcceso || "..."}
          </p>
          <button
            onClick={copiarCodigo}
            className="ml-auto flex items-center gap-1.5 rounded-lg bg-brand/10 px-3 py-1.5 text-xs font-semibold text-brand-400 hover:bg-brand/20 transition-colors"
          >
        ...
        {links.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {links.slice(0, 5).map((link) => (
                ...
                className="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-base/60 transition-colors group"
              >
