    ...
    <div className="relative inline-block text-left">
      ...
      {abierto && (
        <>
          ...
          <div
            className="absolute right-0 z-20 mt-2 w-44 overflow-hidden rounded-xl border border-[#2a2e3a] shadow-2xl shadow-black/60"
            style={{ backgroundColor: "#13151c" }}
          >
            <button onClick={() => { setAbierto(false); onEditar(); }} className={item} style={{ color: "#cbd1e0" }}>
              ✏️ Editar
            </button>
            <button onClick={() => { setAbierto(false); onRol(); }} className={item} style={{ color: "#cbd1e0" }}>
              🔁 Cambiar rol
            </button>
            <button onClick={() => { setAbierto(false); onEstado(); }} className={item} style={{ color: usuario.activo ? "#fb923c" : "#4ade80" }}>
              {usuario.activo ? "⏸️ Desactivar" : "▶️ Activar"}
            </button>
            <button onClick={() => { setAbierto(false); onEliminar(); }} className={item} style={{ color: "#f87171" }}>
              🗑️ Eliminar
            </button>
          </div>
        </>
            ...
            <tbody className="divide-y divide-slate-800">
              {usuariosFiltrados.map((u) => (
                <tr key={u._id} className="bg-slate-950/50 transition-colors hover:bg-slate-900/50">
                  ...
                  <td className="px-4 py-2.5 text-center">
                    <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-medium ${u.activo ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"}`}>
                      <span className={`h-1.5 w-1.5 rounded-full ${u.activo ? "bg-green-400" : "bg-red-400"}`} />
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex justify-end">
                      <MenuAcciones
                        usuario={u}
                        onEditar={() => setUsuarioEditando(u)}
                        onRol={() => manejarCambiarRol(u)}
                        onEstado={() => manejarCambiarEstado(u)}
                        onEliminar={() => manejarEliminar(u)}
                      />
                    </div>
                  </td>
                </tr>
...
EOFcat > ~/OneDrive/Desktop/stockalert-api/stockalert-frontend/src/pages/UsuariosPage.jsx << 'EOF'
import EmptyState from "../components/EmptyState";
import { SkeletonTabla } from "../components/Skeleton";
import { useState } from "react";
import { Navigate } from "react-router-dom";
